...
"""
def runCInstance(jsonrequest,outQueue):
    ...
    if len(stderr)>0:
        for i in range(len(stderr)):
            stderr[i] = stderr[i].replace("test/%s"%('CSolution_%s.c' % uid),"Tests").replace("test/%s"%('CSolution_%s.h' % uid),"Solution")
        stderr =  "\n".join(stderr)
